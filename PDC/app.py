    ...
    if 'loggedin' in session:
        ...
        if request.method == 'POST' and 'id1' in request.form and 'id2' in request.form and'id3' in request.form and'id4' in request.form and'id5' in request.form and'id6' in request.form and 'id7' in request.form and 'id8' in request.form:
            ...
            if marksheet and marksheet.filename != '' and '.' in marksheet.filename and marksheet.filename.rsplit('.', 1)[1].lower() in ALLOWED_EXTENSIONS:
                ...
                marksheet.save(os.path.join(app.config['ACADEMICS'], username + extension))
                ...
                if cursor.rowcount==0:
                    cursor.execute('INSERT into academics(uid,id1,id2,id3,id4,id5,id6,id7,id8, marksheet) values (%s,%s, %s,%s,%s,%s,%s,%s,%s,%s)', (username,sem1,sem2,sem3,sem4,sem5,sem6,sem7,sem8,username+extension))
                    mysql.connection.commit()
                elif cursor.rowcount!=0:
                    sql_update_query = """Update academics set id1=%s,id2=%s,id3=%s,id4=%s,id5=%s,id6=%s,id7=%s,id8=%s,marksheet=%s  where uid = %s"""
                    inputData = (sem1,sem2,sem3,sem4,sem5,sem6,sem7,sem8,username+extension,username)
...
def api_research():
    ...
    username=session['username']
    cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    cursor.execute('INSERT into research (uid, title, mentor, domain, outcome, abstract) values (%s, %s,%s, %s,%s,%s)', (username,title, mentor, domain, outcome, abstract))
    mysql.connection.commit()
    last_id = cursor.lastrowid
    print(last_id)
...
def api_patent():
    ...
    username=session['username']
    cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    cursor.execute('INSERT into patent (uid, title, mentor, domain, outcome, abstract)  values (%s, %s,%s, %s,%s,%s)', (username,title, mentor, domain, outcome, abstract))
    mysql.connection.commit()
    last_id = cursor.lastrowid
    print(last_id)
...
def api_co():
    ...
    #Certificate = certi.filename
        ...
        #certi.save(os.path.join(app.config['COC'], username + extension))
    cursor.execute('INSERT into cocirricular (uid,name,date,type,award,location,certificate) values (%s, %s,%s, %s,%s,%s,%s)', (username,Name_of_activity,Date_of_activity,Type_of_activity,Awards,Location,Certificate))
    mysql.connection.commit()
    last_id = cursor.lastrowid
    print(last_id)
...
def api_extra():
    ...
    username=session['username']
    cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    cursor.execute('INSERT into extracirricular (uid,name,date,type,award,location,certificate) values (%s, %s,%s, %s,%s,%s,%s)', (username,Name_of_activity,Date_of_activity,Type_of_activity,Awards,Location,Certificate))
    mysql.connection.commit()
    last_id = cursor.lastrowid
    print(last_id)
...
def api_social():
    ...
    username=session['username']
    cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    cursor.execute('INSERT into social(uid,name,date,nature,certificate) values (%s, %s,%s, %s,%s)', (username,Name_of_activity,Date_of_activity,Type_of_activity,Certificate))
    mysql.connection.commit()
    last_id = cursor.lastrowid
    print(last_id)
   
    return render_template('table-row3.html', a = Name_of_activity,
        b = Date_of_activity,
        c = Type_of_activity,
        d = Certificate,
        e = last_id)
